...
var Container = function Container() {
    ...
    var _options = _.clone(DEFAULT_OPTIONS);
    ...
    _this.configure = function(options) {       
        ...
        debug('Configuring with [%s]', options);

        // Important to leave storing the options till after
        // higher level difile's have run.
        options = _.extend(DEFAULT_OPTIONS, options || {});
        _optionStack.push(_.clone(options));
        if (options.configfile) {
            var dir = (!options.basedir ? process.cwd() : options.basedir);
            var files = _fileUpSearcher.find(dir);
            files.forEach(function(filename) {
                debug('Loading [%s]...', filename);
                require(filename);
            });
        }
        _options = _optionStack.pop();
        debug('Setting options to be [%s]', _options);
        return _this;
    }
