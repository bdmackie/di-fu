...
function Container(options, state) {
    ...
    _private.resolveImport = function(component, ref, filename) {
        ...
        if (_.isString(importer)) {
            importer = _private.resolveWildcard(importer, filename, ref.bindOptions);
    ...
    _private.resolveWildcard = function(key, filename, bindOptions) {
        if (key.indexOf('*') < 0)
          return key;
        var wildcard = bindOptions && bindOptions.wildcard ? bindOptions.wildcard : helper.stem;
